...
**Before adding a new fetch, check whether the page already has the data.** The value you need is often already in state elsewhere on the page — under a different endpoint, field name, or a sibling component's fetch — and can be derived/filtered client-side instead of triggering another request.

**`/openapi.yaml` can drift from the handler in either direction:** a documented field the handler doesn't actually set, or a field the handler returns that the schema doesn't declare. If a field doesn't behave as documented, read the handler to find out which side is wrong before building against it.
