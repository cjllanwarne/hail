...
- **Filter the already-fetched job list client-side instead of re-fetching per job group.** `GET .../batches/{id}/jobs?recursive=true` returns every job in the batch (recursive isn't the default — omit it and you silently only get root-group jobs); a job group's own jobs are just `jobs.filter(j => j.job_group_id === id)`, no extra request.
